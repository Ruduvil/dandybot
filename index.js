...
const CATEGORIES = [
  ...
  },
  {
    ...
    channels: [
      ...
      { name: '💡suggestions',          type: ChannelType.GuildForum, verifiedOnly: true },
    ]
  },
  {
    name: '📜 RULES',
...
const isStaff = (member) =>
  ...
  member.id === client.application?.owner?.id;
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName === 'setup') {
    ...
    try {
      ...
      // Give owner role to command user
      const ownerMember = await guild.members.fetch(interaction.user.id);
      await ownerMember.roles.add(ownerRole).catch(() => {});

      await interaction.followUp({
        content: '✅ **Servidor montado!**\nUsa `/verify` para verificar membros e `/rolemenu` para postar o menu de roles.',
        ephemeral: true
      });
  ...
  if (commandName === 'verify') {
    if (!isStaff(member)) return interaction.reply({ content: '❌ Só Wardens podem verificar.', ephemeral: true });

    const target = interaction.options.getMember('utilizador');
    const floor  = interaction.options.getInteger('floor');

    const verifiedRole = getRole(guild, '✅ Verified Runner');
  ...
  // ── /role ─────────────────────────────────────────────────────────────────
  if (commandName === 'role') {
    if (!hasRole(member, '✅ Verified Runner')) {
      return interaction.reply({ content: '❌ Precisas de ser verificado primeiro.', ephemeral: true });
    }

    const roleName = interaction.options.getString('nome');
    const role = getRole(guild, roleName);
    if (!role) return interaction.reply({ content: '❌ Role não encontrada no servidor.', ephemeral: true });
  ...
  if (commandName === 'rolemenu') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        '**GAMEPLAY**\n' +
        '`/role 🎯 Distractor` • `/role 🔧 Extractor` • `/role 💊 Support`\n' +
        '`/role 🗡️ Frontliner` • `/role 👁️ Recon` • `/role 🛡️ Protector`\n\n' +
        '**TOON FAVORITO**\n' +
        '`/role 🐱 Little Mime` • `/role 🌿 Sprout` • `/role 🐚 Shelly`\n' +
        '`/role 📺 Broken TV` • `/role 🎈 Astro` • `/role 🧸 Goob`\n' +
        '`/role 🐱 Cosmo` • `/role 🌸 Bobette` • `/role 🐶 Vee` • `/role 🎀 Fly`\n\n' +
        '**PERSONALIDADE**\n' +
        '`/role 🏆 Floor Legend` • `/role 🎲 Chaos Agent` • `/role 💀 Twisted Bait`\n' +
        '`/role ⚡ Active` • `/role 🎨 Decorator`'
      )
      .setFooter({ text: "Dandy's World Elite" });
  ...
  if (commandName === 'runresult') {
    if (!hasRole(member, '✅ Verified Runner')) {
      return interaction.reply({ content: '❌ Verificados apenas.', ephemeral: true });
    }
    ...
    if (!resultsChannel) return interaction.reply({ content: '❌ Canal não encontrado.', ephemeral: true });
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Postado por', value: interaction.user.username, inline: true },
      )
      .setTimestamp();
  ...
  if (commandName === 'kick') {
    ...
    return interaction.reply({ content: `👢 ${target.user.username} foi kickado.`, ephemeral: true });
  }
  ...
  if (commandName === 'ban') {
    ...
    return interaction.reply({ content: `🔨 ${target.user.username} foi banido.`, ephemeral: true });
  }
  ...
  if (commandName === 'help') {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '/verify',            value: 'Verifica um membro e atribui badge de floor *(warden only)*' },
        { name: '/role',              value: 'Escolhe a tua role de gameplay ou toon favorito' },
        { name: '/rolemenu',          value: 'Posta o menu de roles *(warden only)*' },
        { name: '/runresult',         value: 'Posta um resultado de run' },
...
client.on('guildMemberAdd', async (member) => {
  ...
  setTimeout(async () => {
    ...
    if (!fresh.roles.cache.some(r => r.name === '✅ Verified Runner')) {
      ...
      if (log) log.send(`🚪 **${member.user.tag}** removido automaticamente por não verificar em 48h.`);
    }
  }, 48 * 60 * 60 * 1000);
});
